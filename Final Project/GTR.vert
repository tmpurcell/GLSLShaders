...
float uExplosionRadius = 1.0; // Radius of the explosion area
...
void main() {
    ...
    // Check if the time is past the explosion time
    if (Timer >= explodeTime) {
        modifiedPosition.x += Timer * 4.0; // Basic movement
        ...
        if (distanceFromExplosion < uExplosionRadius) {
            ...
            vec3 direction = normalize(modifiedPosition.xyz - EXPLOSION_CENTER); // Updated direction
        ...
        modifiedPosition.x += Timer * 4.0; // Continue basic movement
    }
